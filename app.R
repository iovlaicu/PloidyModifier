...
#library(stringr)
#library("rlist")
source("/home/antonia/PloidyModifier/plotSunriseM.R")
reslocal <- NULL
  ...
  title="ASCAT.scFit",   theme = shinytheme("united"),
      ...
      color: #FFFFFF ; border-radius: 30px; background-color: #3e0533; border-color: #6a0144; padding:5px; font-size:5vh; border-width: 5px;
          ...
          "code {
                ...
                #######################################################################
                img(src='Bioinfo2.png', align = "center", style="width:100%; max-width:100%; position: absolute; z-index:-1;"),
                ...
                fluidRow(column(6, align="center", offset=3, 
                     ...
                     fluidRow(column(6, align="center", offset = 3,
                                     ...
                                     actionButtonStyled("start",
                                              ...
                                              width = "300px"
                                              #status="primary",
                                              #style="color: #FFFFFF ; border-radius: 30px; background-color: #3e0533; border-color: #6a0144; padding:5px; font-size:4vh; border-width: 5px; height:150px; width:300px; opacity: 0.90;"
                                              #buttonType = "primary", size= "lg"
                                              ))
...
server <- function(input, output, session) {
  ...
  observeEvent(input$start, {
    ...
    tryCatch({
      ...
      output$sunrise2 <- renderPlot({isolate( localOpt <<- plotSunriseM(reslocal$allSolutions.refitted.manual[[1]]))}) 
      #print(localOpt)
  ...
  observeEvent(input$discard, {
    ...
    if(! is.null(sampleName())){
      ...
      output$sunrise2 <- renderPlot({isolate(plotSunriseM(reslocal$allSolutions.refitted.auto[[index]]))})
      ...
      tryCatch(
        {
          ...
          purity <- coords$y
          ploidy <- coords$x
          #print(paste0("Purity : ",purity, " , Ploidy : ", ploidy))
          if (optValue()){
            best <- 1
          ...
          output$sunrise2 <- renderPlot({isolate(plotSunriseM(reslocal$allSolutions.refitted.manual[[index]]))})
          coords$y <<- NULL
          coords$x <<- NULL
             },
        error=function(e) {
      ...
      withCallingHandlers(
        {
          ...
          shiftp <- as.numeric(shiftv())
            ...
            output$sunrise2 <- renderPlot({plotSunriseM(reslocal$allSolutions.refitted.manual[[index]])})
  ...
  observeEvent(input$view,{
    ...
    if(! is.null(sampleName())){
      index <- getIndex(sampleName())
       ...
       output$sunrise2 <- renderPlot({isolate( localOpt <<- plotSunriseM(reslocal$allSolutions.refitted.manual[[index]]))})
